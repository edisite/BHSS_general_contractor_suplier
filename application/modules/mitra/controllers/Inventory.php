...
class Inventory extends Admin_Controller {
	...
	public function __construct() {
		parent::__construct();
		$files = array(            
			'assets/js/penerimaan_barang_scripts.js',               
		);
	...
	public function StokGudang(){
            ...
            $this->mPageTitle = "Stok Barang";
            $this->render('inventory/V_stok_gudang');		
	}
	public function BarangSiapKirim(){
		$this->mPageTitlePrefix = 'Gudang - ';
		$this->mPageTitle = "Barang Keluar";
		$this->render('inventory/V_gudang_out');		
	}
	...
	public function loadData($type=''){
		...
		$where_like['data'][] = array(
			'column' => 'jenis_barang_nama, barang_kode, barang_nama, satuan_nama, gudang_nama',
		...
		$query_filter = $this->mod->select($select, 'v_stok_gudang', NULL, NULL, NULL, $where_like, $order);
		$query = $this->mod->select($select, 'v_stok_gudang', NULL, NULL, NULL, $where_like, $order, $limit);

		$response['data'] = array();
		if ($query<>false) {
			$no = $limit['start']+1;
			foreach ($query->result() as $val) {
				if(empty($val->stok_minimum)){
					$stok_minimum = 0;
				}else{
					$stok_minimum = $val->stok_minimum;
				}
				if(empty($val->stok_gudang)){
					$stok_gudang = 0;
				}else{
	...
	public function loaddata_gudang_out(){
		// $priv = $this->cekUser(31);
		$select = '*';
		//LIMIT
		$limit = array(
		...
		//WHERE LIKE
		$where_like['data'][] = array(
			'column' => 'gout_kode, project_id, gout_tgl, jual_kode, status_name',
			'param'	 => $this->input->get('search[value]')
		);
		//ORDER
		$index_order = $this->input->get('order[0][column]');
		...
		$query_total = $this->mod->select($select, 'v_gudang_out');
		$query_filter = $this->mod->select($select, 'v_gudang_out', NULL, NULL, NULL, $where_like, $order);
		$query = $this->mod->select($select, 'v_gudang_out', NULL, NULL, NULL, $where_like, $order, $limit);
		...
		if ($query<>false) {
			...
			foreach ($query->result() as $val) {
				...
				$response['data'][] = array(
					...
					$val->jual_kode,
					$val->project_id,
					'<span class="badge badge-default badge-primary">'.strtolower($val->status_name).'</span>',
					''
				);
				$no++;
			}
	...
	public function getForm(){
		...
		if ($form->validate())
		{
			...
			$in_jml_itemBarang 				= $this->input->post('jml_itemBarang') ?: '';
