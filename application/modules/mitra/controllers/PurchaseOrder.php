...
class PurchaseOrder extends Admin_Controller {
	...
	public function loadData(){
		...
		$where_like['data'][] = array(
			'column' => 'order_nomor, supp_nama, order_tanggal, order_status_nama',
		...
		$query_filter = $this->mod->select($select, 'v_order', NULL, NULL, NULL, $where_like, $order);
		$query = $this->mod->select($select, 'v_order', NULL, NULL, NULL, $where_like, $order, $limit);
	...
	public function getForm(){
		...
		if ($form->validate())
		{
			...
			$in_tanggal_order 			= $this->input->post('tanggal_order') ?: date('d F, Y');
			$in_nama_sales 				= $this->input->post('nama_sales') ?: '';
			$in_m_mitra_supplier_select = $this->input->post('m_mitra_supplier_select') ?: '';
			$in_spp_project 			= $this->input->post('spp_project') ?: '';
			...
			$in_qty 					= $this->input->post('orderdet_qty') ?: '';
			$in_orderdet_harga_satuan	= $this->input->post('orderdet_harga_satuan') ?: '';
			$in_orderdet_total			= $this->input->post('orderdet_total') ?: '';			
			$in_order_subtotal			= $this->input->post('order_subtotal') ?: '';
			$in_order_diskon_input		= $this->input->post('order_diskon_input') ?: '';
			$in_order_diskon			= $this->input->post('order_diskon') ?: '';
			$in_order_tax_input			= $this->input->post('order_tax_input') ?: '';
			$in_order_tax				= $this->input->post('order_tax') ?: '';
			$in_order_other_cost		= $this->input->post('order_other_cost') ?: '';

			$in_order_total				= $this->input->post('order_total') ?: '';
			
			$in_tanggal_order 			= DateTime::createFromFormat('d F, Y', $in_tanggal_order);
			$in_tanggal_order 			= $in_tanggal_order->format('Y-m-d');

			$data = array(
				'order_nomor'						=> $this->get_kode_transaksi(),
				'order_tanggal'						=> $in_tanggal_order,
				'order_referensi_id'				=> $in_spp_project,
				'm_supplier_id'						=> $in_m_mitra_supplier_select,
				'project_id'						=> $in_m_mitra_supplier_select,
				'order_nama_dikirim'				=> $in_nama_sales,
				'order_subtotal'					=> $in_order_subtotal,
				'order_diskon_persen'				=> $in_order_diskon_input,
				'order_diskon_nominal'				=> $in_order_diskon,
				'order_tax_persen'					=> $in_order_tax_input,
				'order_tax_nominal'					=> $in_order_tax,
				'order_other_cost'					=> $in_order_other_cost,
				'order_total'						=> $in_order_total,
				'order_created_date'				=> date('Y-m-d H:i:s'),
				'order_created_by'					=> $this->session->userdata('identity'),
				'order_status'						=> '1',
			);

			$insert = $this->mod->insert_data_table('t_order', NULL, $data);
			if($insert->status) {
				for ($i = 0; $i < $in_jml_item; $i++) {						
						$data_detail = array(
							't_order_id' 			=> $insert->output,
							'm_barang_id' 			=> $in_m_barang_id[$i],
							'orderdet_qty' 			=> $in_qty[$i],
							'orderdet_harga_satuan'	=> $in_orderdet_harga_satuan[$i],
							'orderdet_total'		=> $in_orderdet_total[$i],
							'orderdet_created_date'	=> date('Y-m-d H:i:s'),
							'orderdet_created_by'	=> $this->session->userdata('identity'),
							'orderdet_update_date'	=> date('Y-m-d H:i:s'),
							'orderdet_revised' 		=> 0,
						);

					$insert_detail = $this->mod->insert_data_table('t_orderdet', NULL, $data_detail);
					if($insert_detail->status) {
						// update PB

						$wherePB['data'][] = array(
							'column' => 't_permintaan_pembelian_id',
							'param'	 => $in_spp_project
						);
						$wherePB['data'][] = array(
							'column' => 'm_barang_id',
						...
						$query_pbdet = $this->mod->select('*', 't_permintaan_pembeliandet', NULL, $wherePB);
						if($query_pbdet){
							foreach ($query_pbdet->result() as $row) {
								$jml_realisas = $row->permintaan_pembeliandet_qty_realisasi;
								if(empty($jml_realisas)){
									$jml_realisas = 0;
								}
								$jml_realisas = $jml_realisas + $in_qty[$i];
								$jml_pb = $row->permintaan_pembeliandet_qty;

							}
						}
						if($jml_realisas >= $jml_pb){
							$status = 3;
						}else{
							$status = 2;
						}
						...
						$dataPB = array(
							'permintaan_pembeliandet_qty_realisasi'	=> $in_qty[$i],
							'permintaan_pembeliandet_status' 		=> $status,
							'permintaan_pembeliandet_update_date'	=> date('Y-m-d H:i:s'),
							'permintaan_pembeliandet_update_by'		=> $this->session->userdata('identity'),							
						);
						$updateStok2 = $this->mod->update_data_table('t_permintaan_pembeliandet', $wherePB, $dataPB);
					} 
				}
			}
			$this->system_message->set_success('PO berhasil dibuat');
			refresh();
		}
		$this->mViewData['form'] = $form;
		$this->mPageTitlePrefix = 'Pembelian - ';
		$this->mPageTitle = "Purchase Order";
		$this->render("purchase-order/V_form_purchase_order");
